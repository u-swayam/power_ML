...
6. Model Evaluation: Model performance was evaluated using Mean Absolute Percentage Error (MAPE) to measure prediction accuracy relative to actual demand values. The Linear Regression model yielded a MAPE of 7.7%, indicating limited capability in capturing nonlinear relationships and complex temporal dependencies. The baseline XGBoost model significantly improved performance, achieving a MAPE of 3.14%, demonstrating its effectiveness in modelling nonlinear interactions and feature dependencies. Further hyperparameter tuning and refinement led to an improved XGBoost model with a reduced MAPE of 3.11%
